...
export const Header = () => {
	...
	return (
		<header className={`sticky top-0 z-50 w-full flex justify-center items-center py-3`}>
			<div
				className={`flex items-center justify-between w-full max-w-5xl px-4 py-1 rounded-full shadow-lg`}
				style={{
					minHeight: 60,
					border: `3px solid ${isDark ? '#045294' : '#045294'}`,
					background: isDark ? '#07081a' : 'white',
				}}
			>
				{/* Logo */}
				<Link to="/" className={`flex items-center justify-center h-12 w-12  mr-2`}>
					<Logo className="h-8 w-8 text-white" darkMode={isDark} />
				</Link>
				{/* Desktop Nav */}
				...
				<div className="flex items-center gap-2 md:ml-4">					<button
					...
					<Button className={`hidden md:inline-block ml-2 px-6 py-2 font-bold rounded-full border-2 shadow-md transition-all duration-200 ${isDark ? 'bg-primary text-white border-border hover:bg-primary/90' : 'bg-primary text-white border-white hover:bg-primary/90'}`} style={{ fontSize: "1.1rem" }} asChild>
